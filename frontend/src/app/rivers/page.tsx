...
export default function FloodInformationPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setError(null);
      // Fetch APIs independently to handle failures gracefully
      const [threatResult, riverResult] = await Promise.allSettled([
        api.getFloodThreat(),
        api.getRiverLevels(),
      ]);

      // Handle flood threat data
      if (threatResult.status === 'fulfilled') {
        setFloodThreat(threatResult.value);
      } else {
        console.error('Failed to fetch flood threat:', threatResult.reason);
      }
